...
export default function EditStakeholders() {
  ...
  const { id } = useParams();
  const { stakeholders, stakeholderError, stakeholderLoading, editStakeholder } = useStakeholders();
  const settings = useSettingsContext();
  const stakeholder = stakeholders.details.find(s => s.id === Number(id));
  const mdUp = useResponsive('up', 'md');
  const [name, setName] = useState(stakeholder ? stakeholder.name : '');
  const [nameError, setNameError] = useState(false);
  const [des, setDes] = useState(stakeholder ? stakeholder.des : '');
  const [org, setOrg] = useState(stakeholder ? stakeholder.organization : '');
  const [orgError, setOrgError] = useState(false);
  const [contact, setContact] = useState(stakeholder ? stakeholder.contact : '');
  const [contactError, setContactError] = useState(false);
  const [role, setRole] = useState(stakeholder ? stakeholder.role : '');
  const [roleError, setRoleError] = useState(false);
  const [cat, setCat] = useState(stakeholder ? stakeholder.category : '');
  const [country, setCountry] = useState(stakeholder ? stakeholder.country : '');
  const [lang, setLang] = useState(stakeholder ? stakeholder.language : '');
  ...
  const handleSubmit = (e) => {
    ...
    if (checkData()){
      ...
      editStakeholder(stakeholder).then( () => {
          navigate(-1)
        }
      )
    }
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'lg'}>
      <PageTitle title={"Edit Stakeholder"} />
